...
class FleetTest extends \Tester\TestCase
{
    ...
    /** @var \ondrs\Vehico\Api\PrivateApi\Fleet\FleetApi */
    private $fleetApi;
    ...
    function setUp()
    {
        ...
        $this->fleetApi = new \ondrs\Vehico\Api\FleetApi\FleetApi(TEMP_DIR, $apiKey, 'https://localhost/vehico/vehico-server/www/api');
        */
        ...
        $this->fleetApi = new \ondrs\Vehico\Api\PrivateApi\Fleet\FleetApi(TEMP_DIR, $apiKey, 'https://test.vehico.cz/api');
    ...
    {
        $vehicles = $this->fleetApi->findVehicles();
